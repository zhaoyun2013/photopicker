...
import com.zhaoyun.photopicker.adapters.FloderAdapter;
import com.zhaoyun.photopicker.adapters.PhotoAdapter;
import com.zhaoyun.photopicker.adapters.PhotoAdapter.PhotoCheckedCallBack;
import com.zhaoyun.photopicker.beans.Photo;
import com.zhaoyun.photopicker.beans.PhotoFloder;
import com.zhaoyun.photopicker.utils.LogUtils;
import com.zhaoyun.photopicker.utils.OtherUtils;
import com.zhaoyun.photopicker.utils.PhotoUtils;
 ...
 * @Date: 2016/8/30
 */
public class PhotoPickerActivity extends Activity implements PhotoCheckedCallBack {
    ...
    private void getPhotosSuccess() {
        ...
        mPhotoNumTV.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < mPhotoAdapter.getmSelectedPhotos().size(); i++) {
                    Photo photo = new Photo(mPhotoAdapter.getmSelectedPhotos().get(i));
                    photo.setId(i);
                ...
                bundle.putSerializable(PreviewActivity.EXTRA_DATA, datas);
                bundle.putInt(PreviewActivity.EXTRA_CURRENT_INDEX, 0);
                bundle.putInt(PreviewActivity.EXTRA_MAX_NUM, mMaxNum);
...
//        mPhotoAdapter.setPhotoClickCallBack(this);
        mPhotoAdapter.setPhotoCheckCallBack(this);
    ...
    public void onPhotoClick(int index) {
        ArrayList<Photo> datas = new ArrayList<Photo>();
        List<String> selectedPhotos = mPhotoAdapter.getmSelectedPhotos();
        for (int i = 0; i < mPhotoLists.size(); i++) {
            Photo photo = mPhotoLists.get(i);
            photo.setId(i);
        ...
        Bundle bundle = new Bundle();
        bundle.putSerializable(PreviewActivity.EXTRA_DATA, datas);
        bundle.putInt(PreviewActivity.EXTRA_CURRENT_INDEX, index);
        bundle.putInt(PreviewActivity.EXTRA_MAX_NUM, mMaxNum);
        intent.putExtras(bundle);
        startActivityForResult(intent, REQUEST_PREVIEW);
    }
...
//        Bundle bundle = new Bundle();
//        bundle.putSerializable(PreviewActivity.EXTRA_DATA, datas);
//        bundle.putInt(PreviewActivity.EXTRA_CURRENT_INDEX, mCurrentIndex);
//        bundle.putInt(PreviewActivity.EXTRA_MAX_NUM, mMaxNum);
//        intent.putExtras(bundle);
//        startActivityForResult(intent, REQUEST_PREVIEW);
//    }
    ...
    @Override
    public void onPhotoChecked() {
        LogUtils.d(TAG, "onPhotoClick");
        ...
        if(list != null && list.size()>0) {
            ...
            mPhotoNumTV.setTextColor(getResources().getColor(R.color.preview_enabled));
            mPhotoNumTV.setText(OtherUtils.formatResourceString(getApplicationContext(),
                    R.string.preview_num, list.size(), mMaxNum));
        } else {
            mCommitBtn.setEnabled(false);
            mCommitBtn.setText(R.string.commit);
            mPhotoNumTV.setEnabled(false);
            mPhotoNumTV.setTextColor(getResources().getColor(R.color.preview));
            mPhotoNumTV.setText(R.string.preview);
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if(requestCode == REQUEST_CAMERA){
            ...
            if(resultCode == Activity.RESULT_OK) {
                int type = data.getIntExtra(PreviewActivity.EXTRA_TYPE,0);
                mSelectList = data.getStringArrayListExtra(PreviewActivity.EXTRA_SELECTED_LIST);
                if(type==1) {
                    returnData();
                }else if(type ==2){
