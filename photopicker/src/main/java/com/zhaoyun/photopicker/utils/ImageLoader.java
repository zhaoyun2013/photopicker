package com.zhaoyun.photopicker.utils;
 ...
 * @Class: ImageLoader
 * @Description:
 * @author: liuling(www.liuling123.com)
 * @Date: 2015/11/5
 */
public class ImageLoader {
    ...
    private void initDiskCache() {
        try {
            File cacheDir = OtherUtils.getDiskCacheDir(PhotoPickerActivity.getContext(), "images");
            if (!cacheDir.exists()) {
                cacheDir.mkdirs();
            }
            mDiskLruCache = DiskLruCache.open(cacheDir, OtherUtils.getAppVersion(PhotoPickerActivity.getContext()),
                    1, 15 * 1024 * 1024);
        } catch (IOException e) {
            e.printStackTrace();
        }
